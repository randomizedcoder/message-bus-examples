...
func main() {
	...
	case "nats", "jetstream", "rabbitmq", "valkey", "mqtt":
		run := map[string]func([]string) error{
			"nats": runNATS, "jetstream": runJetStream, "rabbitmq": runRabbitMQ, "valkey": runValkey, "mqtt": runMQTT,
		}[os.Args[1]]
...
func usage() {
	fmt.Fprintln(os.Stderr, "usage: benchcli <codec|grpc|nats|jetstream|rabbitmq|valkey|mqtt|correctness|clockprobe|report> [flags]")
